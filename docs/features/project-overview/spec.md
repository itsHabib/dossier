...
## Deferred: S3 read cost — the manifest, and the cheaper controls first

This is the one place the design protects a *user's wallet*, not just latency. On `FsStore` reads are free. On an object store (S3 / R2 / GCS) every read is **billed** — per-request (Class A/B operations) plus egress for bytes transferred. A read that fans out to one GET per object turns a single orient call into hundreds of metered requests + the egress to download bodies it only counts; an agent that orients a few times per session across a portfolio then hands the operator a surprising bill. So the controls below are a **cost requirement for the S3 backend**, not an optional optimization — sequenced cheapest-first:

1. **Warm cache (primary; already the cloud design's D5).** Reads serve from a warm local working copy synced from remote — startup is list-keys-only, content is fetched lazily, writes push through. Overview is then just another *reader over the cache*: local-disk cost, not per-call S3 fan-out. The constraint this spec adds for the cloud rollout: **`S3Store` must not be the live *read* path without the warm cache.** Phase 1 can validate S3 write-correctness (the CAS gate) without making remote-direct scans the everyday read path — that window is the bill problem. (Overview at v0 scans live only because `FsStore` is live and local scans are free.)

2. **Stop the double-GET in the S3 list path (cheap; do when S3 matters).** Today `S3Store.list_*` GETs every object **twice** — once for the body, once to re-read for the ETag version — and an orient read discards the version entirely. A read that needs only counts (or only bodies) shouldn't pay a second GET per object for a version nobody uses. Splitting "read value" from "read value + version" roughly **halves the request count** for every list read on S3, independent of the cache or manifest. Smallest change, real money saved; the overview verb makes it worth doing. (Recommended `S3Store` follow-up — out of PR 1, which is `FsStore`-only.)

3. **Per-project manifest (endgame).** A single `.dossier/cache/<slug>/overview.json` (LAYOUT.md's `.dossier/cache/` rebuildable-index convention) maintained on write and read by a **narrow `Store` primitive**, so an overview is **one GET instead of N**. This *is* a trait addition — allowed, just later, gated on S3 going live; the v0 service-layer design doesn't foreclose it. The naïve alternative does **not** work: a `bodies:false` read at the seam still pays full egress (S3 GET has no projection — the existing `with_body:false` project read downloads the whole object and drops the body after), so the saving must come from reading a *separate, smaller* manifest object, not from projecting the big ones.

The manifest carries a write-path invalidation burden (every `phase.*` / `task.*` / `artifact.*` write keeps it current). On `FsStore` that cost is real and the benefit hypothetical, so **defer at v0** — not a permanent "no," just the wrong thing to build before there's a billed backend to protect. Lock the lanes (warm cache as the gate to live-S3 reads; double-GET fix + manifest object as the per-call cost controls) before `S3Store` becomes a live read path.
...
- **D9 — S3 read cost is a first-class constraint.** An orient call must never become a per-object GET fan-out billed to the operator. On object-store backends reads serve from the warm cache (cloud D5) — `S3Store` is not a live *read* path without it — and `project.overview` graduates to a single-GET manifest read; the S3 list path stops double-GETting objects for a discarded version. Never assemble-then-discard on a billed backend (also why the D4 guardrail, if revived, must pre-check from counts, not by hydrating the blob). See §Deferred: S3 read cost.
...
- No cross-project overview — `project.list` serves the portfolio-level read; overview is single-project by design. (An agent orienting across the portfolio stays on `project.list`, not a per-project overview loop — which on a billed S3 backend would multiply the fan-out cost by project count; D9.)
